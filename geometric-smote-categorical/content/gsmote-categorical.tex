...
\title{Geometric SMOTENC \\ \LARGE{A Geometrically Enhanced SMOTE-based Method
for Datasets with Nominal and Continuous Features}}
...
phase where the modifications were applied. For the case of oversampling on
datasets with mixed data types, this distinction is especially relevant since
various oversampling methods modify SMOTE's selection mechanism, all of which
based on euclidean distance.

% Algorithms that modify the selection mechanism


However, the usage of these methods becomes a problem on datasets with mixed
data types since distance computations are no longer valid. 

As discussed in Section~\ref{sec:introduction}, research on resampling methods
with mixed data types is 

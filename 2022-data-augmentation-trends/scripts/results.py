...
from research.utils import load_datasets, generate_paths
import sompy
...
encoder = Pipeline(
    [
        ...
        (
            "doc2vec",
            D2VTransformer(size=25, iter=100, min_count=10, seed=42, workers=8),
        ),
...
if __name__ == "__main__":
    ...
    df, df_journals = [
        data[1] for data in load_datasets(data_path, suffix=".db", target_exists=False)
    ]
    ...
    bmus = bmus.join(df.groupby("docs_bmu").size().rename("docs_count"))

    # Save encoder
    pickle.dump(encoder, open(join(results_path, "encoder.pkl"), "wb"))
    ...
    df_net = pd.concat(
        df_net.apply(lambda row: _document_network_connections(*row), axis=1).tolist()
    )
    ...
    df_net = (
        df_net.groupby(["source", "target"])
        .agg({np.size, np.mean, np.sum})["weight"]
        .reset_index()
        .rename(
            columns={
                "mean": "Avg cites",
    ...
    pickle.dump(output, open(join(results_path, "results.pkl"), "wb"))

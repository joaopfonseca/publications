...
if __name__ == "__main__":
    ...
    df = (
        pd.concat([dat for _, dat in data])
    ...
    with connect(join(DATA_PATH, "literature_preprocessed.db")) as connection:
        df.to_sql("slr", connection, index=False, if_exists="replace")
        df_journals.to_sql("journals", connection, index=False, if_exists="replace")

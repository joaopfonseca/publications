...
In this section\hl{,} we provide a general description of the synthetic data
...
We focus on 2 key conditions for the data generation process, smoothness\hl{,} and
...
condition requires synthetic data generation to occur within local\hl{ E}uclidean
...
In the remaining subsections\hl{,} we will describe the main synthetic data
...
primarily focuses on introducing noise via uniform, Laplace\hl{,} and Gaussian
distributions.
...
Masking modifies the original perturbation\hl{-}based approach by introducing a
...
The Gaussian generative model, despite \hl{being in}frequently used when compared to the
remaining Proba\hl{b}ility Density Function mechanisms discussed in this subsection,
is an essential building block for these mechanisms. In particular, we focus
on the multivariate \hl{G}aussian approach, which follows near-Gaussian
...
A Bayesian network can be thought\hl{ }of as a collection of
...
learned) and when the data is high\hl{-}dimensional, making the sampling process
non-trivial.
...
initial observation selected from $\mathcal{D}$, $x_0$\hl{,} and is used to begin
...
modifying $\lambda$'s range to either decreas\hl{e} its minimum value below zero
or increas\hl{e} its maximum value above one.
...
\hl{is} presented in Figure~\ref{fig:linear-transformations}.
...
hyper\hl{ }spheroid. It is formed using an observation to define the center of
...
the hyper\hl{ }spheroid within a subset of the hypersphere, where $\alpha_{trunc}=0$
...
hyperrectangle mechanism uses an approach similar\hl{ }to
...
two-player game composed of two models, a generator\hl{,} and a discriminator. Both
...
learning (for both regularization and oversampling)\hl{,} and reinforcement
learning.
...
layers that correspond\hl{ }to the learned latent space. It contains two parts,
an encoder\hl{,} and a decoder. The encoder transforms the input data into

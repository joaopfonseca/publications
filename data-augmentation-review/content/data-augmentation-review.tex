...
% TODO: Probability density function estimation + Distribution approximation
% (discuss marginal inference)
...
synthetic data generation, the variational AE appears to be the most popular
approach.
...
as Overall Accuracy (OA) or F1-score. One motivation to do this is the ability to
anticipate the quality of the data after training a ML classifier, which may
be an expensive and time-consuming task. This is a challenging problem, since
the usefulness of synthetic data generators depend on the assumptions imposed
according to the dataset, domain and ML problem.

The GANBLR model~\cite{zhang2021ganblr} was evaluated on three aspects:
(1) ML utility, (2) Statistical similarity, and (3) Interpretability. In
...
generators by comparing synthetic data with the original data, as well as ML
utility metrics. According to \citet{alaa2022faithful}, the evaluation of
generative models should quantify three key aspects of synthetic data:
...
\begin{equation}
    U_p = \frac{1}{N} \Sigma^N_{i=1}{(\hat{p}_i - c)}^2
\end{equation}

Where $N = |\mathcal{D} \cup \mathcal{D}^s|$, $c = \frac{|\mathcal{D}^s|}{N}$
and $\hat{p}_i$ is the estimated propensity score for unit $i$. When a
synthetic dataset is relatively similar to the original dataset, $U_p$ will be
close to zero. When the synthetic dataset is easily distinguishable from the
original dataset, $U_p$ will be close to ${(1-c)}^2$. \citet{dankar2021fake},
established a generally consistent negative correlation between $U_p$ and OA\@.
...
Another popular evaluation approach is the computation of the average distance
among synthetic observations and their nearest neighbors within the original
dataset~\cite{hittmeir2019utility}.

A less common approach to compare the performance of synthetic data generators
is to attempt to replicate the results of studies using synthetic
data~\cite{benaim2020analyzing}.
...
\citet{hittmeir2019utility} adopted an evaluation procedure using a 2-step
approach: Similarity comparison and data utility. One of the evaluations is the
comparison of the features' distributions with synthetic data and the original
data using histogram plots. Another evaluation was the comparison of
correlation matrices via heat map plots.

...
\begin{longtable}{rcccccccc}
    ...
    AE-DA~\cite{feng2020autuencoder} & DA & AE & Network \\
    Masking~\cite{yoon2020vime} & DA & Mask & Randomized \\
    ...
    GANBLR~\cite{zhang2021ganblr} & & PGM\\
...
The performance of ML models depends on the quality and size of the training
dataset~\cite{Fenza2021, Halevy2009}. The presence of imbalanced and/or small
datasets, target labels incorrectly assigned, outliers and high dimensional
input spaces reduce the prospects of a successful machine learning (ML) model
implementation~\cite{Halevy2009, Domingos2012, Salman2019}. In the case of
deep learning, for example, it is often limited by a natural
inclination to overfitting, label noise memorization and catastrophic
forgetting~\cite{Xie2021}. Regularization techniques are often used to address
these problems.

When the training data is sampled from a fixed data source, it is balanced and
does not contain label noise, the resulting ML classifier is expected to
achieve good generalization performance~\cite{benning2018modern}. However,
...
overfitting~\cite{Bartlett2021}. Regularization methods attempt to address
these limitations. They can be divided into three
...
types of problems and classifiers~\cite{Behpour2019}. Although data
augmentation is commonly used and extensively studied in computer vision and
natural language processing, research on 

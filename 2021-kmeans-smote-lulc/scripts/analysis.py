...
def generate_main_results():
    ...
    wide_optimal["Dataset"] = pd.Categorical(
        wide_optimal.Dataset.astype(str), categories=DATASETS_ORDER, ordered=True
    )
...
def apply_wilcoxon_test(wide_optimal, dep_var, OVRS_NAMES, alpha):
    ...
    for ovr in OVRS_NAMES[:-1]:
        ...
        pvalues.append(
            wilcoxon(
                wide_optimal.loc[mask, ovr], wide_optimal.loc[mask, "K-SMOTE"]
            ).pvalue
        )
...
def generate_statistical_results():
    ...
    for dataset in wide_optimal.Dataset.unique():
        wilcoxon_results = (
            ...
            .set_index("Oversampler")["p-value"]
            .rename(dataset.title())
        )
        wilcoxon_test.append(wilcoxon_results)
    wilcoxon_test = (
        pd.concat(wilcoxon_test, axis=1)
        .T.reset_index()
    ...
    statistical_results = zip(statistical_results_names, (friedman_test, wilcoxon_test))
...
def make_mean_rank_bar_chart():
    ...
    fig, axes = plt.subplots(
        rs.Classifier.unique().shape[0], rs.Metric.unique().shape[0], figsize=(5, 6)
    )

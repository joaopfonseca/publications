...
generator, tested over a 5-fold cross validation method. The best augmentation
policy found is used to train the iteration's classifier in the following
step. \hl{This procedure is described in Algorithm}~\ref{alg:al-framework}.
...
\begin{algorithm}
    ...
    \caption{Proposed Active Learning Framework}\label{alg:al-framework}
    \DontPrintSemicolon% 
    \KwGiven{$t > 1$}
    \KwIn{$\mathcal{D}_{pool}^t$, $\mathcal{D}_{lab}^t$, $f_c$, $f_{acq}$}
    \KwOut{$\mathcal{D}_{train}$}
\end{algorithm}
...
for each class is defined by the \textit{augmentation factor}\hl{, $\alpha_{af}$}, which
...
\begin{algorithm}
    ...
    \caption{G-SMOTE Modified for Data Augmentation in Active
    Learning}\label{alg:g-smote}
    \DontPrintSemicolon% 

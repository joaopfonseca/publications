...
\begin{abstract}
    ...
    AL\hl{, especially outside image classification or NLP}. This paper
    proposes a new AL framework, which relies on the effective use of
    artificial data\hl{. It may be used with any classifier, generation mechanism
    and data type, and can be integrated with multiple other state-of-the-art
    AL contributions}. \hl{This combination is expected to increase the ML
    ...
    in computational time.} \hl{The proposed} method \hl{introduces} a
    ...
    uncertainty-based\hl{ }data generation mechanism. We compare the proposed
    method to the standard framework \hl{and an oversampling-based} active
    learning method for more informed data generation in an AL context\hl{.}
    ...
    efficiency.\footnote{ \hl{All the code and preprocessed data developed for
...
been found~\cite{Sener2018}. Other methods address imbalance\hl{d} learning by
...
\hl{Recently, data augmentation gathered attention among ML researchers for
its effectiveness over a wide range of applications: regularization,
oversampling, semi-supervised learning, self-supervised learning, etc. Data
augmentation generates synthetic observations to complement naturally
occurring observations. It aims to reinforce the definition of a ML
classifier's decision boundary during the learning phase and improve the
generalization of the algorithm. These techniques have the advantage of being
a data level technique (despite the existence of augmentation methods applied
internally in the ML classifier). Therefore, they can be implemented in a way
that will not affect the choice of classifier and does not exclude the usage
of other regularization approaches. In an AL context, the generation of
synthetic data becomes particularly appealing, especially with randomized and
statistical-based approaches; it ensures better model performance with reduced
involvement of a human agent, at the expense of a marginal increase in
computational power. In addition, synthetic data is expected to reduce the
amount labeled data required for a good AL implementation.}
 
\subsection{Motivation and contributions}

\hl{The} usage of data augmentation in AL is not new. The literature found on the
...
\begin{enumerate}
    ...
    \item \hl{U}sing Deep Learning in an iterative setting requires access to
        significant computational power.

    \item \hl{T}hese models tend to use sophisticated data augmentation methods,
        whose implementation may not be accessible to non-sophisticated users.

    \item \hl{They require a significant amount of processing time per
        iteration and are inappropriate for settings with limited time
        budgets.}

    \item \hl{T}he studies found on the topic are specific to the domain,
        classifier, and data augmentation method.
        \hl{In addition, all of the related methods found (except one) focus
...
augmentation algorithms. \hl{These contributions can be summarized as
follows:}

\begin{enumerate}

    \item \hl{We propose a flexible AL framework with pipelined data
    ...
    \item \hl{We use a geometric-based data augmentation method for
    ...
    \item \hl{We provide empirical evidence that the integration of varying data
...
literature that combines AL with data augmentation. \hl{Table}~\ref{tbl:generators}
\hl{describes the notations used throughout the rest of this study.}

\begingroup\small
\begin{longtable}{clccccccc}
    \caption{%
        \hl{Description of all notations and symbols used throughout the
        manuscript.}
...
iterations (\hl{where }$t \in \{1, 2, \ldots, T\}$) and $n$ annotations per iteration, at
...
$n$ observations with highest u\hl{n}certainty from $\mathcal{D}_{pool}^t$ to form
...
\hl{AL techniques may also be used to complement other well-known learning
...
data}~\cite{wu2021data}\hl{. Another related example is machinery fault
diagnostics, where the quality and quantity of the data collected is often
recognized as a bottleneck}~\cite{zhang2023blockchain}\hl{. In this case,
ML-based techniques frequently leverage unlabeled data to improve the
classification performance}~\cite{zhang2021open}\hl{ and rely on manual data
acquisition}~\cite{he2017deep}\hl{. In these examples, the application of an
...
architectures}~\cite{li2017grass}\hl{ or, in the case of text data, semantic
representation networks}~\cite{zheng2021sentence}\hl{. However, understanding
...
\hl{The standard AL model can be complemented with a data augmentation function,
...
% Pipelined approach
\hl{T}he only AL model found that uses data augmentation outside of the computer
...
highest \hl{un}certainty are labeled by the supervisor and
...
\begin{figure}
    ...
    \caption{%
        ...
        modifications are \hl{comprised within the red polygon and }marked
        with a boldface ``C.''
...
repository (see \hl{final remarks regarding data and software availability}).
...
like $Precision = \frac{TP}{TP+FP}$, $Recall = \frac{TP}{TP+FN}$ \hl{(also
...
defined as a list of various possible values.\hl{ The remaining parameters
...
expense of artificial data generation. \hl{Overall, the proposed method
...
observations from the unlabeled dataset\hl{,} (3) understand the gap between
\hl{randomized} data augmentation techniques and neural network/feature
space data augmentation techniques in an AL context better\hl{, (4) explore
...
\hl{Finally, the proposed method may be applied to any classification problem
...
\hl{The experiment was implemented using the Python programming language, along
with the Python libraries}
\href{https://scikit-learn.org/stable/}{Scikit-Learn}~\cite{Pedregosa2011},
\href{https://imbalanced-learn.org/en/stable/}{Imbalanced-Learn}~\cite{JMLR:v18:16-365},
\href{https://geometric-smote.readthedocs.io/en/latest/?badge=latest}{Geometric-SMOTE}~\cite{Douzas2019},
...
\hl{and}
\href{https://mlresearch.readthedocs.io/en/latest/?badge=latest}{ML-Research}
\hl{libraries. All functions, algorithms, experiments, and results are provided in
the} \href{https://github.com/joaopfonseca/publications/}{project's GitHub

...
and $f_{aug}(x;\tau)$ produces a modified observation, $\tilde{x}$. This
involves the usage of $\mathcal{D}_{train} = \mathcal{D}_{lab} \cup
\mathcal{D}_{aug}$ 
...
query strategy. The few recent AL contributions implementing data augmentation
were all proposed either in the computer vision or natural language processing
realm. 
...
% Pipelined approach
A drop-in application of data augmentation in AL is described
in~\cite{Fonseca2021}. In this study, the AL model proposed is
applied for tabular data. 

% Look ahead data augmentation
Data Augmentation in AL has been recently explored for different
domains~\cite{Kim2021, Quteineh2020, Li2021framework}.
LADA~\cite{Kim2021}. They test two data augmentation methods, one using
Spatial Transformer Networks~\cite{jaderberg2015spatial}, and another using
the Mixup method.
